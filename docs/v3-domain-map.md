...
## Published CONTENT export

Published CONTENT may be exported into generated `.mdx` files, but the direction is still one-way: Notion is the source of truth and the repository only mirrors published read models. Generated files are marked for safe cleanup, manual `.mdx` is preserved, and unsupported block conversions fail closed rather than quietly dropping page content.
