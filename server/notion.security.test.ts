...
describe("Create Well idempotency store", () => {
  it("replays only a matching payload and rejects a key reused with different content", () => {
    ...
    expect(store.read("visitor:key", firstHash)).toEqual({
      kind: "replay",
      response: { recordId: "topic-1" },
    });
    expect(
      store.read("visitor:key", createPayloadHash({ drop: "Different idea" }))
