...
describe("Create Well public intake safeguards", () => {
  it("rejects topic drops that are too short to be actionable", () => {
    expect(topicWellInputSchema.safeParse({ name: "Too brief", drop: "Not enough detail" }).success).toBe(false);
  });
  ...
  it("limits check-ins to the supported operational vocabulary", () => {
    expect(checkInInputSchema.safeParse({
...
describe("Create Well public cache", () => {
  it("reuses a public response inside the cache window", async () => {
    ...
    let calls = 0;
    const first = await readPublicCache("test", async () => ({ calls: ++calls }));
    const second = await readPublicCache("test", async () => ({ calls: ++calls }));
